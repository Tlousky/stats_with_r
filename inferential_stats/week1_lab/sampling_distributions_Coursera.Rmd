...
Answer: 
(2). 50% of houses in Ames are smaller than 1,499.69 square feet (median is actually 1442).
...
kable( 
    samp1 %>% summarise(
        smpl_mean = mean(area),
        smpl_med  = median(area), 
        smpl_sd   = sd(area),
        smpl_iqr  = IQR(area),
        smpl_min  = min(area), 
        smpl_max  = max(area),
        smpl_q1   = quantile(area, 0.25),  # first quartile, 25th percentile
        smpl_q3   = quantile(area, 0.75)   # third quartile, 75th percentile
    ) 
)
...
```{r sampling-dist}
kable( summary( sample_means50 ) )

ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20)
```

The sampling distribution is highly normal, with an almost identical mean and median.
The sampling mean is also quite close to the population mean, although the median is closer.
...
kable( sample_means_small )
```

So we have 25 replications, and 25 sample means of samples with n=10.
...
kable( paste0( "n = ", nrow(sample_means_small) ) )
```
...
kable(
    ames %>% sample_n( size = 15 ) %>% summarize( smpl_mean = mean( price ) ) 
)
...
sample_means15 = ames %>% rep_sample_n( size = 15, rep = 2000, replace = TRUE ) %>% summarize( X_bar = mean( price ) )

ggplot( data = sample_means15, aes( x = X_bar ) ) +
    geom_histogram( binwidth = 2500 )

kable( summary( sample_means15 ) )
```
...
sample_means150 = ames %>% rep_sample_n( size = 150, rep = 2000, replace = TRUE ) %>% summarize( X_bar = mean( price ) )

ggplot( data = sample_means150, aes( x = X_bar ) ) +
    geom_histogram( binwidth = 2500 )

kable( summary( sample_means150 ) )
...
se15  = sd( sample_means15$X_bar,  na.rm = TRUE ) / sqrt( nrow( sample_means15 ) )
se150 = sd( sample_means150$X_bar, na.rm = TRUE ) / sqrt( nrow( sample_means150 ) )

paste0( "1. se15 < se150: ", se15 < se150 )
```
